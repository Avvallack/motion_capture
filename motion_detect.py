...
def get_movement_segments(source: str, processed_files_path: str = './recordings',
                          frames_to_persist=FRAMES_TO_PERSIST,
                          min_size_for_movement=MIN_SIZE_FOR_MOVEMENT,
                          movement_detected_persistence=MOVEMENT_DETECTED_PERSISTENCE,
                          debug_flag=False):
    ...
    while True:
        ...
        if debug_flag:
            cv2.putText(frame, f"{source_basename} @ {millis_to_str(millis)}", (10, 35), font, 0.75, (255, 255, 255), 2,
                        cv2.LINE_AA)
...
def process_pipeline(source_path: str,
                     destination_path: str = './recordings',
                     frames_to_persist=FRAMES_TO_PERSIST,
                     min_size_for_movement=MIN_SIZE_FOR_MOVEMENT,
                     movement_detected_persistence=MOVEMENT_DETECTED_PERSISTENCE,
                     debug_flag=False):
    if os.path.isdir(source_path):
        ...
        get_movement_segments(source_path, destination_path,
                              frames_to_persist, min_size_for_movement,
                              movement_detected_persistence, debug_flag)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--test_flag", action=argparse.BooleanOptionalAction)
    parser.add_argument("--frames", type=int, default=FRAMES_TO_PERSIST)
    parser.add_argument("--min_size", type=int, default=MIN_SIZE_FOR_MOVEMENT)
    parser.add_argument("--persistence", type=int, default=MOVEMENT_DETECTED_PERSISTENCE)
    args = parser.parse_args()
    process_pipeline(args.source_dir, args.destination_dir, args.frames, args.min_size, args.persistence,
                     args.test_flag)